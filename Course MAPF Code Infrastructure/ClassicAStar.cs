...
{   
    ...
    public class ClassicAStar : ICbsSolver, IMStarSolver
    {
        ...
        public virtual void OutputStatistics(TextWriter output)
        {
            ...
            {
                Program.expandedNodesFullAstar = this.GetExpanded();
                Program.generatedNodesFullAstar = this.GetGenerated();
            }
            ...
            {
                Program.expandedNodesStartingAstar = this.GetExpanded();
                Program.generatedNodesStartingAstar = this.GetGenerated();
            }
            ...
            {
                Program.expandedNodesReverseAstar = this.GetExpanded();
                Program.generatedNodesReverseAstar = this.GetGenerated();
                Program.expandedNodesBiDirectionalAstar = Program.expandedNodesStartingAstar + Program.expandedNodesReverseAstar;
                Program.generatedNodesBiDirectionalAstar = Program.generatedNodesStartingAstar + Program.generatedNodesReverseAstar;
            }
        ...
        protected virtual List<WorldState> ExpandOneAgent(List<WorldState> intermediateNodes, int agentIndex)
        {
            ...
            foreach (var currentNode in intermediateNodes)
            {
                ...
                foreach (TimedMove agentLocation in currentNode.allAgentsState[agentIndex].lastMove.GetNextMoves())
                {
                    ...
                    // ********Some of the project modifications: ********
                    // enlarge the variable of numberOpenNodes
                    // numberOfRun 1 is the first solver of the MAPF problem
                    if (Program.numberOfRun == 1)
                    {
                        Program.numberOpenNodesFullAstar += 1;
                    }
                    // numberOfRun 2 is the second run of the solver -> on the first half of the problem (from the start until the middle positions)
                    if (Program.numberOfRun == 2)
                    {
                        Program.numberOpenNodesStartingAstar += 1;
                    }
                    // NumberOfRun 3 is the third run of the solver -> on the second half of the problem (from the end goal state backward to the middle positions state)
                    if (Program.numberOfRun == 3)
                    {
                        Program.numberOpenNodesReverseAstar += 1;
                    }
                }
            }
