...
namespace CPF_experiment
{
    ...
    public class Plan
    {
        ...
        {
            int numberOfMoves = 0;
            int middlePointOfMoves = 0;

            foreach (List<Move> locationsAtTime in this.locationsAtTimes)
            {
                numberOfMoves++;
            }
            Console.Write("\nNumber of moves in the final solution: {0}\n", numberOfMoves);
            //Console.Write("Number of moves in the final solution: {0}\n", middlePointOfMoves);

            // ********Some of the project modifications: ********
            // counter variable is counting the steps progress
            int counter = 0;
            // middlePointOfMoves - this is the middle step of the solution path
            middlePointOfMoves = (numberOfMoves / 2 + 1);
            String middleStage = "|";
            foreach (List<Move> locationsAtTime in this.locationsAtTimes)
            {
                counter++;
                ...
                // Check if the current step is the the middle state of the MAPF solution path       
                if (counter == middlePointOfMoves) {
                    if (Program.numberOfRun == 1) {
                        // Print the middle state of the solution path
                        Console.Write("<-- the middle of the plan");
                    }
                    // Saving the middle state positions - from this the two half Bi-Directional problems will be created
                    foreach (Move aMove in locationsAtTime)
                    {
                        middleStage = middleStage + aMove.ToString() + "|";
                        int[] tempPosition = {0,0};
                        tempPosition[0] = aMove.x;
                        tempPosition[1] = aMove.y;
                        Program.middlePosition.Add(tempPosition);
                    }
                }
                Console.WriteLine();
            }
            if (Program.numberOfRun == 1)
            {
                Program.positionsOfMiddleStage = middleStage;
            }
        }
